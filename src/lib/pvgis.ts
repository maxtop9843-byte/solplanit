...
export function validatePvgisRequest(input: unknown): PvgisRequest {
  ...
  if (latitude < -90 || latitude > 90) {
    throw new PvgisError("위도는 -90도에서 90도 사이여야 합니다.", 400, "INVALID_LATITUDE");
  }
  if (longitude < -180 || longitude > 180) {
    throw new PvgisError("경도는 -180도에서 180도 사이여야 합니다.", 400, "INVALID_LONGITUDE");
  }
  if (peakPowerKw <= 0 || peakPowerKw > 100_000) {
    throw new PvgisError("설치 용량은 0보다 크고 100,000kW 이하여야 합니다.", 400, "INVALID_PEAK_POWER");
  }
  if (systemLossPercent < 0 || systemLossPercent > 100) {
    throw new PvgisError("시스템 손실은 0%에서 100% 사이여야 합니다.", 400, "INVALID_LOSS");
  }
  if (tiltDegrees !== undefined && (tiltDegrees < 0 || tiltDegrees > 90)) {
    throw new PvgisError("경사각은 0도에서 90도 사이여야 합니다.", 400, "INVALID_TILT");
  }
  if (azimuthDegrees !== undefined && (azimuthDegrees < -180 || azimuthDegrees > 180)) {
    throw new PvgisError("방위각은 -180도에서 180도 사이여야 합니다.", 400, "INVALID_AZIMUTH");
  }
  ...
  if (
    ...
    moduleTechnology !== "CIS" &&
    moduleTechnology !== "CdTe" &&
    moduleTechnology !== "Unknown"
  ) {
...
export function buildPvgisUrl(request: PvgisRequest): string {
  ...
  if (request.tiltDegrees !== undefined) params.set("angle", String(request.tiltDegrees));
    ...
    try {
      ...
      if (!retryable(response.status) || attempt === MAX_RETRIES) {
        if (response.status === 429) {
          throw new PvgisError("PVGIS 요청이 많아 잠시 후 다시 시도해 주세요.", 503, "PVGIS_RATE_LIMITED");
        }
