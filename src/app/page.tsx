...
export default function HomePage() {
  return (
    <main>
      ...
      <section id="calculator" className="calculatorEntry" aria-labelledby="calculator-title"><div className="sectionIntro"><p className="sectionKicker">간단한 사전 확인</p><h2 id="calculator-title">우리 건물에는 태양광을 얼마나 설치할 수 있을까?</h2><p>정확한 도면이 없어도 괜찮아요. 건물 유형과 대략적인 면적부터 시작해보세요.</p></div><div className="calculatorPanel"><div className="stepHeader"><span>1 / 4 · 건물 정보</span><div className="progressTrack" aria-hidden="true"><i /></div></div><fieldset><legend>어디에 설치할 예정인가요?</legend><div className="typeGrid">{buildingTypes.map((type, index) => <label key={type} className="typeOption"><input type="radio" name="building" defaultChecked={index === 0} /> <span>{type}</span></label>)}</div></fieldset><div className="fieldPreview"><span>설치할 공간의 면적</span><strong>대략적인 면적을 다음 단계에서 입력해요</strong></div><button className="primaryButton panelButton" type="button">내 건물 설치 가능 용량 확인하기</button></div></section>
