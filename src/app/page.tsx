  ...
  return (
    <main>
      ...
      <section className="hero" aria-labelledby="page-title">
        ...
        <div className="heroVisual"><Image className="visualImage" src="https://images.unsplash.com/photo-1509391366360-2e959784a276?auto=format&fit=crop&w=1800&q=85" alt="맑은 날 태양광 패널이 설치된 건물 지붕" fill priority fetchPriority="high" sizes="(max-width: 980px) calc(100vw - 32px), min(58vw, 835px)" /><div className="capacityChip" aria-label="예상 설치 용량 약 23.4킬로와트"><span>이 건물의 예상 설치 용량</span><strong>약 23.4kW</strong></div></div>
      </section>
      ...
      <section id="cases" className="caseSection" aria-labelledby="cases-title"><div className="sectionHeaderRow"><div><p className="sectionKicker">실제 설치 사례</p><h2 id="cases-title">비슷한 건물의 선택을 먼저 살펴보세요</h2></div><Link className="secondaryLink" href="/cases">모든 사례 보기 →</Link></div><div className="caseGrid"><Link className="caseCard" href="/cases/hwaseong-factory-48kw"><Image className="caseImage" src="https://images.unsplash.com/photo-1508514177221-188b1cf16e9d?auto=format&fit=crop&w=1400&q=80" alt="태양광 패널이 설치된 공장 지붕" fill sizes="(max-width: 900px) calc(100vw - 48px), 690px" /><span className="caseOverlay" aria-hidden="true" /><div><span>공장·창고</span><h3>경기 화성 48.6kW</h3><p>넓은 지붕을 활용한 자가소비형 설치</p></div></Link><Link className="caseCard" href="/cases/asan-house-6kw"><Image className="caseImage" src="https://images.unsplash.com/photo-1497440001374-f26997328c1b?auto=format&fit=crop&w=1200&q=80" alt="태양광 패널이 설치된 주택" fill sizes="(max-width: 900px) calc(100vw - 48px), 510px" /><span className="caseOverlay" aria-hidden="true" /><div><span>주택</span><h3>충남 아산 6.2kW</h3><p>전기요금 절감을 목표로 한 소규모 설치</p></div></Link></div></section>
