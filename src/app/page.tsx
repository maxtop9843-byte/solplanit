    ...
    <main>
      <header className="siteHeader" aria-label="주요 탐색">
        ...
        <nav className="desktopNav" aria-label="주요 메뉴">
          <a href="#calculator">설치 알아보기</a><a href="#cases">설치 사례</a><a href="#quote">질문·견적</a><a href="#experts">전문가 찾기</a>
        </nav>
      ...
      <section className="hero" aria-labelledby="page-title">
        <div className="heroCopy">
          ...
          <div className="heroActions"><a className="primaryButton" href="#calculator">무료로 확인하기</a><a className="secondaryLink" href="#cases">실제 설치 사례 보기 <span aria-hidden="true">→</span></a></div>
        </div>
      ...
      <section id="calculator" className="calculatorEntry" aria-labelledby="calculator-title">
        <div className="sectionIntro"><p className="sectionKicker">간단한 사전 확인</p><h2 id="calculator-title">우리 건물에는 태양광을 얼마나 설치할 수 있을까?</h2><p>정확한 도면이 없어도 괜찮아요. 건물 유형과 대략적인 면적부터 시작해보세요.</p></div>
        <div className="calculatorPanel">
          <div className="stepHeader"><span>1 / 4 · 건물 정보</span><div className="progressTrack" aria-hidden="true"><i /></div></div>
          <fieldset><legend>어디에 설치할 예정인가요?</legend><div className="typeGrid">{buildingTypes.map((type, index) => <label key={type} className="typeOption"><input type="radio" name="building" defaultChecked={index === 0} /> <span>{type}</span></label>)}</div></fieldset>
          <div className="fieldPreview"><span>설치할 공간의 면적</span><strong>대략적인 면적을 다음 단계에서 입력해요</strong></div>
          <button className="primaryButton panelButton" type="button">내 건물 설치 가능 용량 확인하기</button>
        </div>
      </section>

      <section className="resultPreview" aria-labelledby="result-title">
        <div><p className="sectionKicker">결과는 한눈에</p><h2 id="result-title">이 조건이라면 이렇게 예상돼요</h2><p>입력한 정보를 바탕으로 설치 용량부터 발전량, 절감액까지 순서대로 보여드려요.</p></div>
        <div className="resultCards"><article className="resultCard primaryResult"><span>추천 설치 용량</span><strong>23.4 <small>kW</small></strong><p>약 42장의 패널 기준</p></article><article className="resultCard"><span>연간 예상 발전량</span><strong>28,460 <small>kWh</small></strong><p>지역별 일사량 반영</p></article><article className="resultCard"><span>월 예상 절감액</span><strong>약 38 <small>만원</small></strong><p>자가소비 기준 예시</p></article></div>
      ...
      <section id="cases" className="caseSection" aria-labelledby="cases-title"><div className="sectionHeaderRow"><div><p className="sectionKicker">실제 설치 사례</p><h2 id="cases-title">비슷한 건물의 선택을 먼저 살펴보세요</h2></div><a className="secondaryLink" href="#calculator">내 조건 계산하기 →</a></div><div className="caseGrid"><article className="caseCard caseFactory"><div><span>공장·창고</span><h3>경기 화성 48.6kW</h3><p>넓은 지붕을 활용한 자가소비형 설치</p></div></article><article className="caseCard caseHouse"><div><span>주택</span><h3>충남 아산 6.2kW</h3><p>전기요금 절감을 목표로 한 소규모 설치</p></div></article></div></section>

      <section id="quote" className="communitySection" aria-labelledby="community-title"><div><p className="sectionKicker">질문과 견적</p><h2 id="community-title">계산으로 부족한 부분은 전문가에게 물어보세요</h2><p>비슷한 조건의 설치 사례를 찾아보고, 계산 결과를 첨부해 일반 사용자와 전문가에게 질문할 수 있어요.</p><a className="secondaryLink" href="#">커뮤니티 둘러보기 →</a></div><div className="communityTopics" aria-label="커뮤니티 주제"><span>설치 전 질문</span><span>견적 검토</span><span>설치 후기</span><span>전문가 답변</span><span>실제 발전량</span></div></section>
