...
export default function GenerationCalculator() {
  ...
  async function calculate() {
    ...
    setIsLoading(true); setError(null); setAnnualGenerationKwh(null); setMonthlyGeneration([]); setReferenceDate(null); setCalculatedAt(null);
    try {
      const location = REGIONS[region];
      const response = await fetch("/api/pvgis", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ latitude: location.latitude, longitude: location.longitude, peakPowerKw: capacity, systemLossPercent: 14, useHorizon: true, radiationDatabase: "PVGIS-SARAH3" }) });
      ...
      setAnnualGenerationKwh(annual.value.annualGenerationKwh); setMonthlyGeneration(monthly); setReferenceDate(annual.metadata.referenceDate ?? null); setCalculatedAt(annual.metadata.calculatedAt ?? null);
    } catch (caught) {
  ...
  return <div className={styles.calculator}>
    ...
    <section className={styles.resultPanel} aria-live="polite" aria-labelledby="generation-result-title">
      ...
      {annualGenerationKwh !== null ? <><dl className={styles.metrics}><div className={styles.primaryMetric}><dt>연간 예상 발전량</dt><dd>{number.format(annualGenerationKwh)}kWh</dd></div>{monthlyGeneration.map((item) => <div key={item.month}><dt>{item.month}월</dt><dd>{number.format(item.generationKwh)}kWh</dd></div>)}</dl><div className={styles.method}><h3>계산 기준과 출처</h3><p>JRC PVGIS 5.3에서 선택한 지역 중심 좌표와 설치 용량을 사용했습니다. 간단 계산에서는 시스템 손실 14%, 지평선 반영, PVGIS-SARAH3와 최적 경사·방위 계산을 적용합니다.</p><p>검증일: {referenceDate ?? "확인된 정보 없음"}</p><p>조회 시각: {calculatedAt ?? "확인된 정보 없음"}</p><p><a href="https://re.jrc.ec.europa.eu/pvg_tools/en/" target="_blank" rel="noreferrer">JRC PVGIS 공식 도구</a></p></div></> : null}
    </section>
  </div>;
}
