...
export default function ProWorkspace() {
  ...
  return (
    <main className="proShell">
      ...
      <section className="projectBar" aria-label="프로젝트 요약"><div><p className="proEyebrow">새 프로젝트</p><h1>태양광 발전량 분석</h1></div><dl><div><dt>위치</dt><dd>{Number(latitude).toFixed(3)}, {Number(longitude).toFixed(3)}</dd></div><div><dt>데이터 소스</dt><dd>PVGIS 5.3 · {radiationDatabase}</dd></div><div><dt>상태</dt><dd><span className="statusDot" />{isAnalyzing ? "분석 중" : validationMessage ? "입력 확인 필요" : result ? "분석 완료" : "분석 준비"}</dd></div></dl></section>
      <section className="workspace" aria-label="전문가 분석 워크스페이스">
        ...
        <aside className="controlPane" aria-label="분석 입력"><div className="tabs" role="tablist" aria-label="입력 항목">{tabs.map((tab) => <button key={tab} type="button" role="tab" aria-selected={activeTab === tab} onClick={() => setActiveTab(tab)}>{tab}</button>)}</div>
          ...
          {activeTab === "경제성" && <div className="emptyPanel" role="tabpanel"><strong>경제성 가정</strong><p>설치비, 전기 가치, SMP·REC 등 확인 가능한 값을 직접 입력하는 구조는 발전량 결과 이후 연결됩니다.</p></div>}
          <p id="analysis-status" className={validationMessage || analysisError ? "validationMessage" : "readyMessage"} role="status">{validationMessage ?? analysisError ?? (result ? "PVGIS 5.3 분석 결과를 불러왔습니다." : "필수 입력이 유효합니다. 분석 실행 시 PVGIS 5.3 서버 프록시로 전달됩니다.")}</p>
        </aside>
