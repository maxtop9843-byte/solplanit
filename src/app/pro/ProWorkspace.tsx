...
export default function ProWorkspace() {
  ...
  return (
    <main className="proShell">
      ...
      <section className="projectBar" aria-label="프로젝트 요약"><div><p className="proEyebrow">{importedCalculation ? "일반 사용자 계산에서 가져옴" : "새 프로젝트"}</p><h1>태양광 발전량 분석</h1>{importedCalculation && <div className="importedSummary" role="status"><span>소비자 계산 결과</span><p>추천 용량 {formatNumber(importedCalculation.capacityKw, 1)}kW{importedCalculation.panelCount !== null ? ` · 패널 약 ${formatNumber(importedCalculation.panelCount)}장` : ""}{importedCalculation.annualGenerationKwh !== null ? ` · 연간 예상 ${formatNumber(importedCalculation.annualGenerationKwh)}kWh` : ""}</p><small>가져온 조건은 수정할 수 있으며, 분석을 시작하기 전 최종 확인이 필요합니다.</small></div>}</div><dl><div><dt>위치</dt><dd>{Number(latitude).toFixed(3)}, {Number(longitude).toFixed(3)}</dd></div><div><dt>데이터 소스</dt><dd>PVGIS 5.3 · {radiationDatabase}</dd></div><div><dt>상태</dt><dd><span className="statusDot" />{isAnalyzing ? "분석 중" : validationMessage ? "입력 확인 필요" : result ? "분석 완료" : importedCalculation ? "가져오기 완료" : "분석 준비"}</dd></div></dl></section>
