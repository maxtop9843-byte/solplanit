...
function readGeneralCalculation(search: string): ImportedCalculation | null {
  ...
  return {
    capacityKw,
    panelCount: parseOptionalNumber(params.get("panels")),
    annualGenerationKwh: parseOptionalNumber(params.get("generation")),
    annualBenefit: parseOptionalNumber(params.get("benefit")),
    paybackYears: parseOptionalNumber(params.get("payback")),
    goal: goal === "save" || goal === "sell" ? goal : null,
  };
}

function parseAnalysisResult(payload: unknown): AnalysisResult {
  const proxy = recordValue(payload);
  const data = recordValue(proxy.data);
  const outputs = recordValue(data.outputs);
  const totals = recordValue(recordValue(outputs.totals).fixed);
  const monthlyRaw = recordValue(outputs.monthly).fixed;
  const horizonRaw = outputs.horizon_profile;
  const monthly = Array.isArray(monthlyRaw) ? monthlyRaw.map((item, index) => {
    const row = recordValue(item);
    return {
      month: numberValue(row.month) ?? index + 1,
      productionKwh: numberValue(row.E_m) ?? 0,
      irradiationKwhM2: numberValue(row["H(i)_m"]) ?? 0,
      variationKwh: numberValue(row.SD_m) ?? 0,
    };
  }).filter((item) => item.month >= 1 && item.month <= 12) : [];
  const horizon = Array.isArray(horizonRaw) ? horizonRaw.map((item) => {
    const row = recordValue(item);
    return { azimuth: numberValue(row.A) ?? 0, height: numberValue(row.H_hor) ?? 0 };
  }) : [];
  const annualProductionKwh = numberValue(totals.E_y);
  const annualIrradiationKwhM2 = numberValue(totals["H(i)_y"]);
  if (annualProductionKwh === null || annualIrradiationKwhM2 === null || monthly.length === 0) {
    throw new Error("PVGIS 응답에서 필수 발전량 결과를 확인하지 못했습니다.");
  }
  return {
    annualProductionKwh,
    annualIrradiationKwhM2,
    annualVariationKwh: numberValue(totals.SD_y) ?? 0,
    totalLossPercent: numberValue(totals.l_total) ?? 0,
    monthly,
    horizon,
    source: typeof proxy.source === "string" ? proxy.source : "PVGIS",
    version: typeof proxy.version === "string" ? proxy.version : "5.3",
    verifiedAt: typeof proxy.verifiedAt === "string" ? proxy.verifiedAt : "—",
    retrievedAt: typeof proxy.retrievedAt === "string" ? proxy.retrievedAt : new Date().toISOString(),
  };
}
...
export default function ProWorkspace() {
  ...
  const [importedCalculation, setImportedCalculation] = useState<ImportedCalculation | null>(null);
  const [result, setResult] = useState<AnalysisResult | null>(null);
  const [analysisError, setAnalysisError] = useState<string | null>(null);
  const [isAnalyzing, setIsAnalyzing] = useState(false);

  const validationMessage = useMemo(() => {
    const lat = Number(latitude); const lng = Number(longitude); const power = Number(peakPowerKw);
    const loss = Number(systemLossPercent); const tilt = Number(tiltDegrees); const azimuth = Number(azimuthDegrees);
  ...
  useEffect(() => {
    ...
    if (!imported) return;
    setImportedCalculation(imported);
    setPeakPowerKw(String(imported.capacityKw));
  }, []);
  ...
  async function runAnalysis() {
    if (validationMessage || isAnalyzing) return;
    ...
    try {
      ...
      if (!response.ok) {
        const message = recordValue(recordValue(payload).error).message;
        throw new Error(typeof message === "string" ? message : "PVGIS 분석을 완료하지 못했습니다.");
      }
      setResult(parseAnalysisResult(payload));
    } catch (error) {
      setResult(null); setAnalysisError(error instanceof Error ? error.message : "PVGIS 분석을 완료하지 못했습니다.");
    } finally { setIsAnalyzing(false); }
  }

  const maxMonthlyProduction = Math.max(...(result?.monthly.map((item) => item.productionKwh) ?? [1]), 1);
  const maxHorizonHeight = Math.max(...(result?.horizon.map((item) => item.height) ?? [1]), 1);
    ...
    <main className="proShell">
      ...
      <section className="workspace" aria-label="전문가 분석 워크스페이스">
        ...
        <aside className="controlPane" aria-label="분석 입력"><div className="tabs" role="tablist" aria-label="입력 항목">{tabs.map((tab) => <button key={tab} type="button" role="tab" aria-selected={activeTab === tab} onClick={() => setActiveTab(tab)}>{tab}</button>)}</div>
          ...
          {activeTab === "경제성" && <div className="emptyPanel" role="tabpanel"><strong>경제성 가정</strong><p>{importedCalculation?.annualBenefit !== null && importedCalculation ? `일반 계산의 연간 예상 ${importedCalculation.goal === "sell" ? "발전 수익" : "절감액"} ${formatNumber(importedCalculation.annualBenefit)}원을 참고값으로 가져왔습니다. 전문 분석 결과와 직접 비교해 확인하세요.` : "설치비, 전기 가치, SMP·REC 등 확인 가능한 값을 직접 입력하는 구조는 발전량 결과 이후 연결됩니다."}</p></div>}
