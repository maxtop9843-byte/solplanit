...
// 내려받는 이미지와 인쇄 보고서는 페이지 밖에서 열리므로 CSS 변수를 쓸 수 없다.
// DESIGN.md 2절 무채색 값을 그대로 옮겨 적는다. 값이 바뀌면 여기도 함께 바꾼다.
const EXPORT = {
  canvas: "#F2F2F4",
  surface: "#FDFDFD",
  ink: "#0F1012",
...
function chartSvg(result: DownloadResult) {
  ...
  const bars = result.monthly.map((item, index) => {
    ...
    return `<g><rect x="${x}" y="${y}" width="${barWidth}" height="${barHeight}" fill="${EXPORT.ink}"/><text x="${x + barWidth / 2}" y="${y - 12}" text-anchor="middle" font-size="18" fill="${EXPORT.ink}">${Math.round(item.productionKwh).toLocaleString("ko-KR")}</text><text x="${x + barWidth / 2}" y="${chartBottom + 34}" text-anchor="middle" font-size="18" fill="${EXPORT.inkMuted}">${MONTHS[item.month - 1]}</text></g>`;
  }).join("");
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}"><rect width="100%" height="100%" fill="${EXPORT.canvas}"/><text x="64" y="70" font-family="Arial, sans-serif" font-size="34" font-weight="400" fill="${EXPORT.ink}">SolPlanit 월별 발전량</text><text x="64" y="112" font-family="Arial, sans-serif" font-size="20" fill="${EXPORT.inkMuted}">${result.source} ${result.version} · 연간 ${Math.round(result.annualProductionKwh).toLocaleString("ko-KR")} kWh</text><line x1="64" y1="${chartBottom}" x2="1140" y2="${chartBottom}" stroke="${EXPORT.hairline}"/>${bars}<text x="64" y="652" font-family="Arial, sans-serif" font-size="16" fill="${EXPORT.inkMuted}">${DISCLAIMER}</text></svg>`;
}
...
export default function ResultDownloads({ result, project }: ResultDownloadsProps) {
  ...
  function printPdf() {
    ...
    report.document.write(`<!doctype html><html lang="ko"><head><meta charset="utf-8"><title>SolPlanit PVGIS 보고서</title><style>body{font-family:Arial,sans-serif;color:${EXPORT.ink};background:${EXPORT.canvas};margin:40px}header{border-bottom:1px solid ${EXPORT.hairline};padding-bottom:18px}h1{margin:4px 0;font-weight:400}.meta,.summary{display:grid;grid-template-columns:repeat(2,1fr);gap:10px;margin:24px 0}.card{border:1px solid ${EXPORT.hairline};border-radius:16px;background:${EXPORT.surface};padding:16px}.card strong{display:block;font-size:24px;font-weight:400;margin-top:6px}table{width:100%;border-collapse:collapse;margin-top:20px}th,td{border-bottom:1px solid ${EXPORT.hairline};padding:9px;text-align:right;font-weight:400}th:first-child{text-align:left}.notice{margin-top:24px;padding:16px;background:${EXPORT.surface};border:1px solid ${EXPORT.hairline};border-radius:16px;font-size:13px;color:${EXPORT.inkMuted}}@media print{button{display:none}}</style></head><body><header><small>SolPlanit 전문가용</small><h1>태양광 발전량 분석 보고서</h1><p>${result.source} ${result.version} · 검증일 ${result.verifiedAt}</p></header><section class="meta"><div class="card">위치<strong>${project.latitude}, ${project.longitude}</strong></div><div class="card">설치 용량<strong>${project.peakPowerKw} kWp</strong></div><div class="card">경사 / 방위<strong>${project.tilt} / ${project.azimuth}</strong></div><div class="card">조회 시각<strong>${new Date(result.retrievedAt).toLocaleString("ko-KR")}</strong></div></section><section class="summary"><div class="card">연간 발전량<strong>${Math.round(result.annualProductionKwh).toLocaleString("ko-KR")} kWh</strong></div><div class="card">연간 일사량<strong>${Math.round(result.annualIrradiationKwhM2).toLocaleString("ko-KR")} kWh/m²</strong></div><div class="card">연간 변동성<strong>± ${Math.round(result.annualVariationKwh).toLocaleString("ko-KR")} kWh</strong></div><div class="card">PVGIS 계산 총손실<strong>${result.totalLossPercent.toLocaleString("ko-KR")} %</strong></div></section><h2>월별 상세</h2><table><thead><tr><th>월</th><th>발전량(kWh)</th><th>일사량(kWh/m²)</th><th>표준편차(kWh)</th></tr></thead><tbody>${monthlyRows}</tbody></table><p class="notice">${DISCLAIMER}<br>출처: ${result.source} ${result.version} · 복사 데이터베이스: ${project.radiationDatabase}</p><script>window.onload=()=>window.print()</script></body></html>`);
