...
function chartSvg(result: DownloadResult) {
  ...
  const bars = result.monthly.map((item, index) => {
    ...
    return `<g><rect x="${x}" y="${y}" width="${barWidth}" height="${barHeight}" rx="8" fill="#16823B"/><text x="${x + barWidth / 2}" y="${y - 12}" text-anchor="middle" font-size="18" fill="#111827">${Math.round(item.productionKwh).toLocaleString("ko-KR")}</text><text x="${x + barWidth / 2}" y="${chartBottom + 34}" text-anchor="middle" font-size="18" fill="#4B5563">${MONTHS[item.month - 1]}</text></g>`;
  }).join("");
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}"><rect width="100%" height="100%" fill="#ffffff"/><text x="64" y="70" font-family="Arial, sans-serif" font-size="34" font-weight="700" fill="#111827">SolPlanit 월별 발전량</text><text x="64" y="112" font-family="Arial, sans-serif" font-size="20" fill="#4B5563">${result.source} ${result.version} · 연간 ${Math.round(result.annualProductionKwh).toLocaleString("ko-KR")} kWh</text><line x1="64" y1="${chartBottom}" x2="1140" y2="${chartBottom}" stroke="#D1D5DB"/>${bars}<text x="64" y="652" font-family="Arial, sans-serif" font-size="16" fill="#6B7280">${DISCLAIMER}</text></svg>`;
}
