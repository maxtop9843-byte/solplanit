...
1. 실제 진입점에서 기능을 찾을 수 있는지 확인한다.
2. 데스크톱 대표 흐름을 처음부터 결과까지 확인한다.
3. 모바일 375px 흐름을 확인한다.
4. 입력 label, 오류, 빈 상태, 로딩, 결과, 다음 행동을 확인한다.
5. 결과 숫자가 질문에 직접 답하는지 확인한다.
6. 일반 사용자에게 모르는 값을 요구하지 않는지 확인한다.
7. Preview가 가능하면 실제 화면을 확인한다.

현재 승인된 Renewal V2 시각 언어는 `DESIGN.md`를 따른다.
