...
- 미해결:
  - Lighthouse 접근성 점수를 측정하지 못했다. 이 환경에서 Lighthouse 를 실행할 수단이
    없다. 대신 WCAG 2.1 AA 항목을 브라우저에서 직접 실측했고 위반은 0건이다.
    배포 후 실제 측정이 필요하다
  - 0.5px 헤어라인은 DPR 1 화면에서 브라우저가 1px 로 스냅한다. 레티나 실측은
    이 환경에서 불가능하다. 선언값은 전부 0.5px 이다

---

## 사용자 확인이 필요한 사항

0. **푸터에 실을 공개 연락처.** `DESIGN.md` 7절 푸터는 이메일 표기를 요구한다.
   세션 환경에 있던 개인 메일 주소를 임의로 공개 페이지에 싣지 않았고, 지금은
   `/trust/contact` 로 링크만 걸어두었다. 공개할 주소를 알려주면 넣는다.
