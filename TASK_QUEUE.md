...
- [OPEN] **REBUILD-001** 검색 의도 중심으로 홈 문구·정보구조·대표 계산기 UX 전면 재설계
  ...
  - 대표 질문은 `우리 집에 태양광을 설치하면 얼마 들고 얼마나 아낄 수 있을까?`를 중심으로 검토한다.
  - 기본 입력은 일반 사용자가 아는 값만 사용한다: 지역, 건물 유형, 월 전기요금 또는 사용량, 필요 시 지붕 면적.
  - `지붕 면적을 모름` 흐름을 제공한다.
  - 경사, 방위, 손실률 등은 자동 데이터 또는 `상세 설정`으로 분리한다.
  - 결과는 설치 가능 용량, 예상 발전량, 예상 절감 효과 등 사용자의 질문에 직접 답하도록 설계한다.
  - 공식 근거가 없는 설치비·보조금은 확정값처럼 보여주지 않는다.
